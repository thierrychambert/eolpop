server <- function(input, output, session){
  ...
  ## (Saether & Engen 2002) : between 7% et 14 ==> average : 10%
  ## (Saether et al. 2005) : between 2.5% et 10% ==> average : 6%
  ...
  observeEvent({
    ...
    updateMatrixInput(session, inputId = "fatalities_mat_cumulated",
                      value =  matrix(fill_mat, nrow = n_farm, ncol = 3, byrow = TRUE,
                                      dimnames = list(paste("Farm", c(1:n_farm)),
                                                      c("Mean",
                                                        "Standard Error",
                                                        "Start year")))
                      )
  ...
  plot_expert <- function(out, show_se = TRUE, ...){
    plot_elicitation(out, ylab = "", xlab = "Parameter value", cex.lab = 1.2, yaxt = "n")
    mtext(text = "Probability density", side = 2, line = 2, cex = 1.2)
    ...
    mtext(text = paste("Mean = ", round(out$mean_smooth,2)), side = 3, line = 2.5, cex = 1.2, adj = 0)
    if(show_se) mtext(text = paste("Standard error = ", round(sqrt(out$var_smooth), 2)), side = 3, line = 1, cex = 1.2, adj = 0)
  }
  ...
  observeEvent({
    ...
    if( all(!is.na(input$fatalities_mat_expert)) ) {
      ...
      ## plot distribution
      output$title_distri_plot <- renderText({ "Annual fatalities" })
      output$distri_plot <- renderPlot({ plot_expert(param$fatalities_eli_result$out) })

    } else {
      param$fatalities_eli_result <- NULL
      print("missing value")
      output$title_distri_plot <- renderText({ "Values are missing in the 'experts' table" })
    } # end if
  }) # end observeEvent
  ...
  observeEvent({
    ...
    if(all(!is.na(input$pop_size_mat_expert))) {
      ...
      ## plot distribution
      output$title_distri_plot <- renderText({ "Population size" })
  ...
  observeEvent({
    ...
    if(all(!is.na(input$pop_growth_mat_expert))){
      ...
      ## plot distribution
      output$title_distri_plot <- renderText({ "Population growth rate" })
  ...
  observeEvent({
    ...
    if(all(!is.na(input$carrying_cap_mat_expert))) {
      ...
      if(!is.na(param$carrying_cap_eli_result$out)){
        output$title_distri_plot <- renderText({ "Carrying capacity" })
        output$distri_plot <- renderPlot({ plot_expert(param$carrying_cap_eli_result$out, show_se = FALSE) })
      }else {
        output$title_distri_plot <- renderText({ "Error : some values in the 'experts' table cannot be interpreted" })
      }
  ...
  plot_gamma <- function(mu, se, show_mode = TRUE, show_mean = TRUE, show_se = TRUE, ...){
    ...
    curve(dgamma(x, shape=shape, scale=scale), from = max(0,mu-3*se), to = mu+3*se, lwd = 3, col = "darkblue", yaxt = "n",
          ylab = "", xlab = "Parameter value", cex.lab = 1.2)
    mtext(text = "Probability density", side = 2, line = 2, cex = 1.2)
    ...
    if(show_mean) mtext(text = paste("Mean = ", round(mu, 2)), side = 3, line = 2.5, cex = 1.2, adj = 0)
    if(show_se) mtext(text = paste("Standard error = ", round(se, 3)), side = 3, line = 1, cex = 1.2, adj = 0)
  ...
  plot_gamma_cumulated_impacts <- function(mu, se, nparc, ...){
    ...
    for(j in 2:nparc){
      curve(dgamma(x, shape=shape[j], scale=scale[j]),
            from = max(0,mu[j]-4*se[j]), to = mu[j]+4*se[j], n = 1e4,
            lwd = 3, col = j, yaxt = "n",
            ylab = "", xlab = "Parameter value", cex.lab = 1.2, add = TRUE)
    ...
    legend(x = xlim[1], y = ylim[2], legend = paste("Farm", 1:nparc),
    ...
    if(input$analysis_choice == "single_farm"){
      ...
      if(input$fatalities_input_type != "eli_exp"){
        ...
        if(length(param$fatalities_se) < 2){
          output$title_distri_plot <- renderText({ "Annual fatalities: no variation/uncertainty in the value provided" })
        }else{
      ...
      if(input$analysis_choice == "cumulated"){
        output$title_distri_plot <- renderText({ "Annual fatalities on each wind farm (cumulative impact)" })
        ...
        output$title_distri_plot <- renderText({ "No plot (no uncertainty in the case of 'hypothetical scenarios')" })
        output$distri_plot <- NULL
      } # end "else"
  ...
  observeEvent({
    ...
    if(input$pop_size_input_type != "eli_exp"){
      # output$title_distri_plot <- renderText({ "Initial population size" })

      req(param$pop_size_se)
      if(param$pop_size_se <= 0){
        output$title_distri_plot <- renderText({ "Initial population size: no variation/uncertainty in the value provided" })
      }else{
        output$title_distri_plot <- renderText({ "Initial population size" })
      }
  ...
  observeEvent({
    ...
    if(input$pop_growth_input_type != "eli_exp" & input$pop_growth_input_type != "trend"){
      ...
      if(param$pop_growth_se <= 0){
        output$title_distri_plot <- renderText({ "Population growth rate: no variation/uncertainty in the value provided" })
      }else{
        output$title_distri_plot <- renderText({ "Population growth rate" })
      }
  ...
  observeEvent({
    ...
    if(input$carrying_cap_input_type != "eli_exp"){
      ...
      if(param$carrying_capacity_se <= 0){
        output$title_distri_plot <- renderText({ "Carrying capacity: no variation/uncertainty in the value provided" })
      }else{
        output$title_distri_plot <- renderText({ "Carrying capacity" })
      }
  ...
  output$fatalities_unit_info <- renderText({
    if(!is.null(input$fatalities_unit)){
      if(input$fatalities_unit == "h"){
        paste0("Mortality rate")
      } else {
        paste0("Number of fatalities")
      }
    }
  })
  ...
  output$pop_size_unit_info <- renderText({
    if(!is.null(param$pop_size_unit)){
      if(param$pop_size_unit == "Npair"){
        ...
        paste0("Total headcount")
      }
    }
  })

  ## VALUES
  output$pop_size_mean_info <- renderText({  paste0("Mean: ", param$pop_size_mean) })
  output$pop_size_se_info <- renderText({  paste0("Standard error: ", param$pop_size_se) })

  ## Show Popsize by age (table)
  # Function to create the table
  make_mat_popsizes <- function(data_sf, species, pop_size, pop_size_unit, survivals, fecundities){
    nam <- data_sf %>%
      filter(Species_en == species) %>%
      select(classes_age) %>%
      unlist %>%
      as.vector
  ...
  delay(ms = 200,
        output$pop_size_by_age <- renderTable({
          if(any(is.na(param$survivals)) | any(is.na(param$fecundities))){
            matrix("Missing values of survival and/or fecundities",
                   nrow = 1, dimnames = list(NULL, "Errorr"))
          }else{
  ...
  ## Population growth
  ##-------------------------------
  output$pop_growth_mean_info <- renderText({  paste0("Mean: ", param$pop_growth_mean) })
  output$pop_growth_se_info <- renderText({  paste0("Standard error: ", param$pop_growth_se) })
  ...
  output$carrying_capacity_unit_info <- renderText({
    if(!is.null(param$pop_size_unit)){
      if(input$carrying_cap_input_type == "no_K"){
        "No carrying capacity (K = infinity)"
      }else{
        if(param$pop_size_unit == "Npair"){
          paste0("Number of pairs")
        } else {
  ...
  output$carrying_capacity_mean_info <- renderText({
    if(input$carrying_cap_input_type == "no_K"){
      ...
      paste0("Mean: ", param$carrying_capacity_mean)
    }
  })
  ...
  output$carrying_capacity_se_info <- renderText({
    if(input$carrying_cap_input_type == "no_K"){
      ...
      paste0("Standard error: ", param$carrying_capacity_se)
    }
  })
  ...
  observeEvent({
    ...
    updateMatrixInput(session, inputId = "mat_fill_vr",
                      value = matrix(data = NA,
                                     ...
                                     ncol = 2,
                                     dimnames = list(c(paste("Age", (1:number_age_class)-1)), c("Survival", "Fecundity"))))
  }) # end observeEvent
  ...
  observeEvent({
    ...
    if(input$species_choice == "Custom species") {
      ...
      if(all(is.na(tab_species))) {
        ...
        updateMatrixInput(session, inputId = "mat_fill_vr",
                          value = matrix(data = c(survivals, fecundities),
                                         nrow = number_age_class,
                                         ncol = 2,
                                         dimnames = list(ages, c("Survival", "Fecundity"))))
      } # end if 2
    } # end if 1
  ...
  delay(ms = 300,
        output$lambda0_info <- renderText({
          ...
          if(taux < 0) Text <- "Decline: " else Text <- "Growth: "
          if(taux == 0) Text <- "Stable population: "
  ...
  observeEvent({
    ...
    distAVG <- species_data %>%
      filter(Species_en == input$species_choice) %>%
  ...
  output$dispersal_mean_info <- renderText({
    paste0("Average distance of dispersal : ", rv$distAVG, " km")
  })

  output$dispersal_d03p_info <- renderText({
    paste0("3% quantile : ", rv$dist[1], " km")
  })

  output$dispersal_d05p_info <- renderText({
    paste0("5% quantile : ", rv$dist[2], " km")
  })

  output$dispersal_d10p_info <- renderText({
    paste0("10% quantile : ", rv$dist[3], " km")
  })
  ...
  observeEvent({
    ...
    if(input$fatalities_unit == "h"){
      pop_size_tot <- sum(pop_vector(pop_size = param$pop_size_mean, pop_size_type = param$pop_size_unit, s = param$s_calibrated, f = param$f_calibrated)[-1])
  ...
  observe(if(input$run > 0 & nclicks() == 0 & !is.null(result_N())) run_message("Analysis completed"))
  ...
  observeEvent({
    ...
    if(ready$fatalities & ready$pop_size & ready$pop_growth & ready$carrying_capacity){
      ...
      progress <- AsyncProgress$new(session, min=1, max=nsim, message = "In preparation")
      ...
      result <- finally(result,
                        function(){
                          ...
                          out$run_time <- paste(round(as.numeric(duration), 2), units(duration))
                          print(out$run_time)
                        })
  ...
  observeEvent(input$status,{
    ...
    if(run_message() != "Running...") run_message("Ready")
  })
  ...
  output$run_time <- renderText({
    ...
    paste("Computing time (simulations) :", out$run_time)
  })
  ...
  print_indiv_impact <- function(N){
    ...
    matrix(fil,
           nrow = n_farm,
           dimnames = list(paste("Farm",1:n_farm), c("Impact", "CI (min)", "CI (max)"))
    )
  }
  ...
  print_impact <- function(N, show_CI){
    ...
    RowNam <- NULL
    if(out$analysis_choice == "single_farm") RowNam <- c("Farm 1")
    if(out$analysis_choice == "cumulated") RowNam <- c("Farm 1", paste("... + Farm", (2:n_scen)))
    ...
    matrix(fil,
           nrow = n_scen,
           dimnames = list(RowNam, c("Impact", "CI (min)", "CI (max)"))
    )
  }
  ...
  table_impact_QT <- function(N, show_quantile){
    ...
    matrix(fil,
           nrow = n_scen,
           dimnames = list(RowNam, c("Impact at quantile"))
    )
  }
  ...
  print_PrExt <- function(N){
    ...
    RowNam <- NULL
    if(out$analysis_choice == "single_farm") RowNam <- c("No wind farm", "With wind farm")
    if(out$analysis_choice == "cumulated") RowNam <- c("No wind farm", "+ Farm 1", paste("... + Farm", (3:n_scen)-1))
  ...
  plot_out_PDF <- function(N, legend_position, text_size, show_scenario, show_CI){
    if(is.null(N)) {} else {
      ...
      n_scen <- dim(N)[3]
      Legend <- NULL
      if(out$analysis_choice == "single_farm") Legend <- c("Farm 1")
      if(out$analysis_choice == "cumulated") Legend <- c("Farm 1", paste("... + Farm", (3:n_scen)-1))
      ...
      density_impact(N = N, show_CI = show_CI, center = "median",
                     ...
                     percent = TRUE, xlab = "\nRelative impact (%)", ylab = "Probability density\n",
  ...
  plot_out_ECDF <- function(N, legend_position, text_size, show_scenario, show_quantile){
    if(is.null(N)) {} else {
      ...
      ECDF_impact(N = N, show_quantile = show_quantile, sel_sc = show_scenario,
                  xlims = c(0,100),
                  percent = TRUE, xlab = "\nRelative impact (%)", ylab = "Cumulative probability density\n",
                  Legend = Legend, legend_position = legend_position, text_size = text_size)
    }
  }
  ...
  plot_out_impact <- function(N, legend_position, text_size, show_scenario, show_CI){
    if(is.null(N)) {} else {
      ...
      n_scen <- dim(N)[3]
      Legend <- NULL
      if(out$analysis_choice == "single_farm") Legend <- c("No wind farm", "With wind farm")
      if(out$analysis_choice == "cumulated") Legend <- c("No wind farm", "+ Farm 1", paste("... + Farm", (3:n_scen)-1))
      if(out$analysis_choice == "multi_scenario") Legend <- paste("Scenario", (1:n_scen)-1)

      plot_impact(N = N, onset_year = param$onset_year, sel_sc = show_scenario,
                  percent = TRUE, show_CI = show_CI,
                  xlab = "\nYear", ylab = "Relative impact (%)\n", Legend = Legend,
  ...
  output$title_impact_result <- renderText({
    req(input$run)
    paste("Estimated impact after" , param$time_horizon, "years")
  })
  ...
  output$title_PrExt_result <- renderText({
    req(input$run)
    paste("Probability of extinction after", param$time_horizon, "years")
  })
  ...
  observe({
    if(!is.null(result_N())){
      ...
      names(choices) <- c("All", paste("Scenario", 1:n_scen))

      updateRadioButtons(session, "show_scenario",
                         label = "Choice of scenario",
                         choices = choices, # c("all", paste("scenario", 1:n_scen)),
                         selected = "all"
      )
    }
  })
  ...
  output$title_PDF_plot <- renderText({
    if(input$run > 0){
      paste("Result: Probability density of the relative impact after", param$time_horizon, "years")
    }
  })
  ...
  output$title_ECDF_plot <- renderText({
    if(input$run > 0){
      paste("Result: Cumulative probability density of the relative impact after", param$time_horizon, "years")
    }
  })
  ...
  output$quantile_impact_result <- renderText({
    ...
    paste0("Scenario ", 1:length(impact_QT), " : ", round(impact_QT,1), "%", collapse = "\n")
  })
  ...
  output$title_impact_plot <- renderText({
    if(input$run > 0){
      "Result: Relative impact across time"
    }
  })
  ...
  output$warning_traj_plot <- renderText({
    if(input$run > 0){
      "Caution: These are predictions based on current knowledge.
      No one can predict how factors that influence a population demography (environment, etc.) will change in the future.
      Therefore, no one can predict exactly what the population size will be in several years.\n
      This graph is only provided as an additional piece of information. Be careful not to over-interpret its meaning.\n
      The impact of collisions should be assessed using relative impact values (and graphs), as they provide a more robust
      (i.e., less sensitive to assumptions and uncertainties) estimate of that impact."
    }
  })
  ...
  observeEvent({
    ...
    if(out$analysis_choice == "single_farm") out$analysis_choice_report <- "Impacts of a single wind farm"
    if(out$analysis_choice == "cumulated") out$analysis_choice_report <- "Cimulative impacts (>1 wind farm)"
    if(out$analysis_choice == "multi_scenario") out$analysis_choice_report <- "Hypothetical scenarios"
  })
  ...
  observeEvent({
    ...
    if(input$fatalities_unit == "M"){
      out$fatalities_unit <- paste0("Unit : annual number of fatalities")
      unit <- " fatality(ies)"
    }
    if(input$fatalities_unit == "h"){
      out$fatalities_unit <- paste0("Unit : annual rate of mortality (%)")
      unit <- " %"
    }
    ...
    if(input$analysis_choice == "multi_scenario"){
      req(input$fatalities_vec_scenario)
      out$fatalities_input_type <- "Entry: Hypothetical scenarios"
      vec01 <- as.numeric(unlist(strsplit(input$fatalities_vec_scenario, " ")))
      out$fatalities_val1 <- paste0("Scenario(s) of mortality: ",
                                    vec01, unit, ".")
      ...
      if(input$analysis_choice == "cumulated"){
        out$fatalities_input_type <- "Entry: cumulative impacts"
        out$fatalities_val1 <- paste0("Estimated value: ", input$fatalities_mat_cumulated[,1], unit, " ; ")
        out$fatalities_val2 <- paste0("Standard error: ", input$fatalities_mat_cumulated[,2], unit)
      }else{
        ## Analysis : single farm
        if(input$fatalities_input_type == "itvl"){
          out$fatalities_input_type <- "Entry: interval"
          out$fatalities_val1 <- paste0("Min: ", input$fatalities_lower, unit, " ; ")
          out$fatalities_val2 <- paste0("Max: ", input$fatalities_upper, unit)
        }
        if(input$fatalities_input_type == "val"){
          out$fatalities_input_type <- "Entry: estimation and standard error"
          out$fatalities_val1 <- paste0("Estimated value: ", input$fatalities_mean, unit, " ; ")
          out$fatalities_val2 <- paste0("Standard error : ", input$fatalities_se, unit)
        }
        if(input$fatalities_input_type == "eli_exp"){
          out$fatalities_input_type <- "Entry: expert elicitation"
          out$fatalities_val1 <- paste0("Estimated mean: ", round(param$fatalities_eli_result$mean, 2), unit, " ; ")
          out$fatalities_val2 <- paste0("Standard error: ", round(param$fatalities_eli_result$SE, 2), unit)
        }
        ##### END single fram ###
      } # if 2: "cumulated"
    } # if 1 : multi_scenario
  ...
  observeEvent({
    ...
    if(input$pop_size_unit == "Npair"){
      out$pop_size_unit <- paste0("Unit: number of pairs")
      unit <- " pairs"
    }
    if(input$pop_size_unit == "Ntotal"){
      out$pop_size_unit <- paste0("Unit: total headcount")
      unit <- " individuals"
    }

    if(input$pop_size_input_type == "itvl"){
      out$pop_size_input_type <- "Entry: interval"
      out$pop_size_val1 <- paste0("Min: ", input$pop_size_lower, unit, " ; ")
      out$pop_size_val2 <- paste0("Max: ", input$pop_size_upper, unit)
    }
    if(input$pop_size_input_type == "val"){
      out$pop_size_input_type <- "Entry: estimation and standard error"
      out$pop_size_val1 <- paste0("Estimated value: ", input$pop_size_mean, unit, " ; ")
      out$pop_size_val2 <- paste0("Standard error: ", input$pop_size_se, unit)
    }
    if(input$pop_size_input_type == "eli_exp"){
      out$pop_size_input_type <- "Entry: expert elicitation"
      out$pop_size_val1 <- paste0("Estimated mean: ", round(param$pop_size_eli_result$mean, 2), unit, " ; ")
      out$pop_size_val2 <- paste0("Standard error: ", round(param$pop_size_eli_result$SE, 2), unit)
    }
  })
  ...
  observeEvent({
    ...
    if(input$pop_growth_input_type == "itvl"){
      out$pop_growth_input_type <- "Entry: interval"
      out$pop_growth_val1 <- paste0("Min: ", input$pop_growth_lower, unit, " ; ")
      out$pop_growth_val2 <- paste0("Max: ", input$pop_growth_upper, unit)
    }
    if(input$pop_growth_input_type == "val"){
      out$pop_growth_input_type <- "Entry: estimation and standard error"
      out$pop_growth_val1 <- paste0("Estimated value: ", input$pop_growth_mean, unit, " ; ")
      out$pop_growth_val2 <- paste0("Standard error: ", input$pop_growth_se, unit)
    }
    if(input$pop_growth_input_type == "eli_exp"){
      out$pop_growth_input_type <- "Entry: expert elicitation"
      out$pop_growth_val1 <- paste0("Estimated mean: ", round(param$pop_growth_eli_result$mean, 2), unit, " ; ")
      out$pop_growth_val2 <- paste0("Standard error: ", round(param$pop_growth_eli_result$SE, 2), unit)
    }

    ## TREND ##
    if(input$pop_growth_input_type == "trend"){
      out$pop_growth_input_type <- "Entry: population trend"
      ...
      if(input$pop_trend == "growth"){
        V2 <- "growth"
        if(input$pop_trend_strength == "weak") V1 <- "Weak"
        if(input$pop_trend_strength == "average") V1 <- "Medium"
        if(input$pop_trend_strength == "strong") V1 <- "Strong"
      }
  ...
  observeEvent({
    ...
    if(input$pop_size_unit == "Npair"){
      out$carrying_cap_unit <- paste0("Unit : number of pairs")
      unit <- " pairs"
    }
    if(input$pop_size_unit == "Ntotal"){
      out$carrying_cap_unit <- paste0("Unit : total headcount")
      unit <- " individuals"
    }

    if(input$carrying_cap_input_type == "itvl"){
      out$carrying_cap_input_type <- "Entry: interval"
    ...
    if(input$carrying_cap_input_type == "val"){
      out$carrying_cap_input_type <- "Entry: estimation and standard error"
      out$carrying_cap_val1 <- paste0("Estimated value: ", input$carrying_capacity_mean, unit, " ; ")
      out$carrying_cap_val2 <- paste0("Standard error: ", input$carrying_capacity_se, unit)
    }
    if(input$carrying_cap_input_type == "eli_exp"){
      out$carrying_cap_input_type <- "Entry: expert elicitation"
      out$carrying_cap_val1 <- paste0("Estimated mean: ", round(param$carrying_cap_eli_result$mean, 2), unit, " ; ")
      out$carrying_cap_val2 <- paste0("Standard error: ", round(param$carrying_cap_eli_result$SE, 2), unit)
    }

    if(input$carrying_cap_input_type == "no_K"){
      out$carrying_cap_input_type <- NULL
      out$carrying_cap_val1 <- paste0("No carrying capacity")
